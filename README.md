...
新版默认“直接阅读，重点复核”：复核先显示疑点分组，表格整体浏览；保存修订立即落盘，点击“更新成品”再统一生成 Word 和报告。“我的任务”可以打开旧书、继续中断任务、查看空间与用量。详见[长书使用与验收记录](docs/long-book-usability-20260916.md)。
...
官网 PDF 每个上传部分最多 200 页、190,000,000 字节。优先采用书签章节、小节和段落附近的页边界，实际落盘测量体积，复制原始页面而不降低图像清晰度。主分片页码不重叠且覆盖完整。分片接缝（包括章节边界）补充解析前后页，只有页码与内容能唯一对应时才修复；不确定的接缝保留主分片内容，在报告中列出页码。
...
完成或继续任务后点击“重点复核”。默认只列出待处理疑点，表格聚合为一个入口，点击具体单元格编辑；关闭“重点复核”后可按章节连续阅读原译文。支持章节、问题类别、处理状态和全文搜索，记住上次位置。公式、图片、代码与参考文献保持结构保护。

- 手动编辑后点击“保存修订”，立即保存并标记“成品待更新”；检查数字及受保护结构，普通疑点可以明确保留，结构损坏不能覆盖。完成一批修改后点击“更新成品”。
- “AI 改译 / 补译”使用原任务的服务与模型、当前本书术语设置，候选单独显示；先采用到编辑框，再保存。此操作产生模型请求，需要原服务可用的 Key。
- “撤销上次译文修改”保留历史；人工修订独立保存，续跑优先使用。术语约束变化后保留人工译文并标记待复核。
- Word 文件被占用等原因导致导出失败时，修订仍已保存；点击“更新成品”不再调用模型。修订历史、迁移备份及成品事务备份均位于 `_internal`，中断的成品替换会在下次任务操作时恢复。
